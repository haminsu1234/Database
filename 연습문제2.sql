...
#실습 2-16
SELECT MAX(t_amount) AS `큰금액` , MIN(t_amount) AS `작은금액` FROM `bank_transaction`
WHERE `t_dist`=2;
...
JOIN `bank_account` AS b ON a.t_a_no = b.a_no
JOIN `bank_customer` AS c ON b.a_c_no=c.c_no
WHERE `c_dist`=1 AND `t_dist` BETWEEN 1 and 2 
GROUP BY `c_no`
...
SET @@sql_mode ='ONLY_FULL_GROUP_BY,_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
